<x-app>
    ...
    <div class="flex items-center w-full justify-center min-h-screen">
        <form method="post" class="max-w-sm mx-auto bg-gray-400 dark:bg-gray-800 p-8 rounded-4xl" action="{{ route("costumer.update", $costumer->id) }}">
            @method('put')
            @csrf
            <div class="w-full flex justify-center mb-8">
                <h2>Editar: {{ $costumer->name }}</h2>
            </div>
            <div class="mb-5">
            ...
            <div class="mb-5">
                <label for="country" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Data de nascimento</label>
                <input type="country" name="country" value="{{ $costumer->country }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    required />
            </div>
